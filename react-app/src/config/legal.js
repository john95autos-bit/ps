/**
 * Legal document copy — the port of config/legal.php.
 *
 ...
 *   }
 *
 * Tokens: paragraph text may contain {legal_name}, {address}, {phone}, {email}
 * and {consent_text}; pages/Legal.jsx substitutes them at render time. Keeping
 * the consent statement a token rather than a second hand-typed copy is a fix
 * for a real drift risk in the PHP build, where config/site.php and the privacy
 * policy each held their own copy of the same TCPA wording and nothing kept
 * them in step.
 *
 * Section 3 of the privacy policy is a function of whether a measurement tag is
 * actually configured. The PHP build stated flatly that "this website loads a
 * Google advertising tag on every page" while no tag id was set, so both the
 * policy and the consent banner described something that was not happening.
 *
 * The "Last updated" date is NOT stored here — it comes from SITE.legalUpdated.
 */

/**
 * @param {boolean} hasTag  true when a valid AW-/GTM- id is configured, so a
 *                          measurement tag really does load on every page.
 */
export function legalDocuments(hasTag) {
  const measurement = hasTag
    ...
    : 'This website does not currently load any advertising, analytics or measurement tag, and it sets no cookies. If that changes, a Google advertising tag would be added under Google Consent Mode and would stay denied — reading and writing no advertising or analytics identifiers — until you chose otherwise in the privacy banner. This section will be updated to say so plainly at that point.';

  const measurementChoice = hasTag
    ...
    : 'Should a tag be added later, choosing "Accept optional" would switch it to granted, allowing it to set and read identifiers to measure whether an advertisement led to an enquiry, including recording that you tapped a phone-call button and which button it was. Choosing "Essential only" would keep it denied.';
  ...
  return {
    ...
    privacy: {
      ...
      intro:
        'This policy explains what this website may collect, why it is used and the choices available to visitors.',
      sections: [
        {
          title: '1. Who we are',
          paragraphs: [
        ...
        {
          /* TCPA. 47 CFR 64.1200(f)(9) requires prior express written consent to
             disclose that consent is not a condition of purchase, and to identify
             the calling party. A bare checkbox without those sentences is weak
             consent, so the quoted statement below is injected from
             LEAD_FORM.consentText — the exact string stored with each submission
          ...
          paragraphs: [
            ...
            'The consent box on the enquiry form is unticked by default, sits immediately above the submit button, and must be ticked before the form will send. It reads: “{consent_text}”',
    ...
    terms: {
      ...
      intro:
        'By using this website, you agree to the following terms. Separate service terms may apply to any work you authorise.',
      sections: [
        {
    ...
    /* -----------------------------------------------------------------------
     * Disclaimer — 8 sections (unnumbered titles).
     * -------------------------------------------------------------------- */
    disclaimer: {
      eyebrow: 'Service disclaimer',
      title: 'Important information before you rely on this site.',
      intro:
        'Please read these limitations alongside the relevant service page and any quotation or agreement.',
      sections: [
        {
